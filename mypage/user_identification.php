        ...
        if($_POST['app_id'] == $app_id && $_POST['app_key'] == $app_key) {//应用拥有访问权限
            ...
            if($nums) { //根据token能查询到,
                ...
                if(strtotime(date('Y-m-d H:i:s'))-strtotime($query_result['timestamp']) < 3600*24*2) {//该token仍在有效期
                    ...
                    $return_data = json_encode(array("is_login"=>"true","u_id"=>$uid));
                }else {//token已经过期,删除token并返回结果
                    mysql_query("DELETE FROM sso_token WHERE token='$token'");
                    $return_data = json_encode(array("is_login"=>"false","u_id"=>-1));
...
//                        die("删除过期token出错!");
                ...
                $return_data = json_encode(array("is_login"=>"false","u_id"=>0));
            }

        } else {
//            die("Access Deny!");
            $return_data = json_encode(array("is_login"=>"deny","u_id"=>0));
